...
function buildrpm
{
  ...
  #Build RPMs for i686
  mock -r fedora-$FEDVER-i686 --rebuild --resultdir=$HOMEDIR/repo/"%(dist)s"/"%(target_arch)s"/os/ $1
}
if [[ $1 = "clean" ]]; then
  rm -rf $HOMEDIR/repo/
  mkdir $HOMEDIR/repo/ $HOMEDIR/repo/lastlogs/ $HOMEDIR/repo/lastlogs/source/ $HOMEDIR/repo/lastlogs/x86_64/ $HOMEDIR/repo/lastlogs/i686/
  for (( ver=18 ; ver<=19 ; ver++ ))
    do
      mkdir $HOMEDIR/repo/fc$ver/ $HOMEDIR/repo/fc$ver/x86_64/ $HOMEDIR/repo/fc$ver/i686/ $HOMEDIR/repo/fc$ver/source
      mkdir $HOMEDIR/repo/fc$ver/x86_64/os/ $HOMEDIR/repo/fc$ver/i686/os/ $HOMEDIR/repo/fc$ver/x86_64/debug/ $HOMEDIR/repo/fc$ver/i686/debug/
#      createrepo $HOMEDIR/repo/fc$ver/source/
#      createrepo $HOMEDIR/repo/fc$ver/x86_64/os/
#      createrepo $HOMEDIR/repo/fc$ver/i686/os/
#      createrepo $HOMEDIR/repo/fc$ver/x86_64/debug/
#      createrepo $HOMEDIR/repo/fc$ver/i686/debug/
    done
  exit
  ...
  #Move last i686 logs to separate directory
  find $HOMEDIR/repo/fc$FEDVER/i686/os/ -type f -name '*.log' -exec mv {} $HOMEDIR/repo/lastlogs/i686/ \;
  #Delete SRPMs from x86_64 non-source repo
  find $HOMEDIR/repo/fc$FEDVER/x86_64/os/ -type f -name '*.src.rpm' -delete
  #Delete SRPMs from i686 non-source repo
  find $HOMEDIR/repo/fc$FEDVER/i686/os/ -type f -name '*.src.rpm' -delete
  ...
  #Move debuginfo i686 to separate repository
  find $HOMEDIR/repo/fc$FEDVER/i686/os/ -type f -name '*debuginfo*' -exec mv {} $HOMEDIR/repo/fc$FEDVER/x86_64/debug/ \;
  createrepo --update $HOMEDIR/repo/fc$FEDVER/source/
  createrepo --update $HOMEDIR/repo/fc$FEDVER/x86_64/os/
  createrepo --update $HOMEDIR/repo/fc$FEDVER/i686/os/
  createrepo --update $HOMEDIR/repo/fc$FEDVER/x86_64/debug/
  createrepo --update $HOMEDIR/repo/fc$FEDVER/i686/debug/
fi
